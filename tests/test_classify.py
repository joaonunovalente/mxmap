...
class TestClassify:
    ...
    def test_us_other_mx(self):
        assert classify(["mxf.eu.mpssec.net"], "") == "us-other"

    def test_us_other_mx_fortimail(self):
        assert classify(["mx01.fortimailcloud.com"], "") == "us-other"

    def test_independent_mx(self):
        assert classify(["mail.example.nl"], "") == "independent"
    ...
    def test_nl_isp_asn(self):
        result = classify(
            ["mail.kpn.nl"],
            "",
            mx_asns={1136},
        )
        assert result == "pt-isp"

    def test_nl_isp_does_not_override_hostname_match(self):
        result = classify(
            ["mail.protection.outlook.com"],
            "",
            mx_asns={1136},
    ...
    def test_nl_isp_does_not_override_cname_match(self):
        result = classify(
            ["mail.example.nl"],
            "",
            mx_cnames={"mail.example.nl": "mail.protection.outlook.com"},
            mx_asns={1136},
        )
        assert result == "microsoft"

    def test_nl_isp_with_autodiscover_microsoft(self):
        """PT ISP relay with autodiscover pointing to outlook.com → microsoft."""
        result = classify(
            ["mail.kpn.nl"],
            "",
            mx_asns={1136},
    ...
    def test_nl_isp_without_autodiscover_stays_nl_isp(self):
        """PT ISP relay without autodiscover stays pt-isp."""
        result = classify(
            ["mail.kpn.nl"],
            "",
            mx_asns={1136},
    ...
    def test_non_nl_isp_asn_stays_independent(self):
        result = classify(
            ["mail.example.nl"],
            "",
            mx_asns={99999},
    ...
    def test_barracuda_gateway_with_google_spf(self):
        result = classify(
            ["mail2.barracudanetworks.com"],
    ...
    def test_independent_mx_webapps_spf_profile_with_aws_asn(self):
        """Independent MX + webapps SPF profile + AWS ASN should classify as aws."""
        result = classify(
            ["mail.mondimdebasto.pt"],
            "v=spf1 include:spf.webapps.net ip4:136.144.240.14 ~all",
            spf_asns={8622, 16509, 20857},
        )
        assert result == "aws"

    def test_ptempresas_mx_classified_as_pt_isp(self):
        result = classify(
            ["mxc.ptempresas.pt", "mxp.ptempresas.pt"],
            "v=spf1 redirect=operations.ptempresas.pt",
        )
        assert result == "pt-isp"

    def test_ovh_mx_classified_as_eu_isp(self):
        result = classify(
            ["mx0.mail.ovh.net"],
            "",
        )
        assert result == "eu-isp"
...
class TestClassifyFromAutodiscover:
    ...
    def test_microsoft_cname_outook_typo(self):
        assert (
            classify_from_autodiscover(
                {"autodiscover_cname": "autodiscover.outook.com"}
            )
            == "microsoft"
        )
    ...
    def test_unrecognized_returns_none(self):
        assert (
            classify_from_autodiscover(
                {"autodiscover_cname": "autodiscover.custom-host.nl"}
            )
            is None
        )

    def test_us_other_fortimail_autodiscover(self):
        assert (
            classify_from_autodiscover(
                {"autodiscover_cname": "autodiscover.fortimail.com"}
            )
            == "us-other"
        )
...
class TestDetectGateway:
    ...
    def test_no_gateway(self):
        assert detect_gateway(["mail.example.nl"]) is None
...
class TestClassifyFromMx:
    ...
    def test_unrecognized_returns_independent(self):
        assert classify_from_mx(["mail.custom.nl"]) == "independent"
...
class TestClassifyFromSpf:
    ...
    def test_unrecognized_returns_none(self):
        assert classify_from_spf("v=spf1 include:custom.nl -all") is None
...
class TestClassifyFromSmtpBanner:
    ...
    def test_google_esmtp_in_ehlo(self):
        assert (
            classify_from_smtp_banner("220 custom.example.nl", "250 Google ESMTP ready")
            == "google"
        )
    ...
    def test_us_other_banner(self):
        assert classify_from_smtp_banner("220 mpssec.net ESMTP") == "us-other"

    def test_postfix_returns_none(self):
        assert classify_from_smtp_banner("220 mail.example.nl ESMTP Postfix") is None

    def test_exim_returns_none(self):
        assert classify_from_smtp_banner("220 mail.example.nl ESMTP Exim 4.96") is None
