...
MANUAL_OVERRIDE_CBS: set[str] = set()
...
def print_report(scored_entries: list[dict[str, Any]]) -> None:
    ...
    print(f"    {'CBS':>8}  {'Score':>5}  {'Provider':<12} {'Name':<30} Flags")
    print(f"    {'-' * 8}  {'-' * 5}  {'-' * 12} {'-' * 30} {'-' * 20}")
    for e in lowest:
        flags_str = ", ".join(e["flags"])
        print(
            f"    {e['cbs']:>8}  {e['score']:>5}  {e['provider']:<12} "
...
def run(data_path: Path, output_dir: Path, quality_gate: bool = False) -> bool:
    ...
    with open(csv_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["cbs", "name", "provider", "domain", "confidence", "flags"])
