...
_SPF_IP4_RE = re.compile(r"\bip4:([0-9]{1,3}(?:\.[0-9]{1,3}){3}(?:/[0-9]{1,2})?)")


def _spf_representative_ips(spf_text: str) -> list[str]:
    """Extract one representative host IP per /24 block from SPF ip4: entries.

    Deduplicates by /24 prefix so large Microsoft/Google CIDR ranges don't
    generate hundreds of Cymru lookups.
    """
    seen: set[str] = set()
    ips: list[str] = []
    for match in _SPF_IP4_RE.finditer(spf_text):
        entry = match.group(1)
        try:
            net = ipaddress.IPv4Network(entry, strict=False)
            hosts = list(net.hosts())
            host = str(hosts[0]) if hosts else str(net.network_address)
        except ValueError:
            continue
        prefix24 = ".".join(host.split(".")[:3])
        if prefix24 not in seen:
            seen.add(prefix24)
            ips.append(host)
    return ips
...
async def resolve_spf_asns(spf_text: str) -> set[int]:
    """Return set of ASNs owning the ip4: blocks in a (resolved) SPF record."""
    asns: set[int] = set()
    for ip in _spf_representative_ips(spf_text):
